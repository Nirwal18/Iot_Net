...
fun DashboardScreen(
    ...
    vm: DashboardScreenViewModel,
    navigateToDeviceTemplate: () -> Unit
) {
            ...
            Box(modifier = Modifier.padding(it)){
                LazyColumn(
                    ...
                    items(state.devices) { device ->
                        DeviceCard(device = device, onCmDBtnClick = onClickDeviceStartCmdBtn, onClick = navigateToDeviceTemplate)
                    }
                }
            }
...
@Composable
fun DeviceCard(device: Device, onCmDBtnClick:(Boolean)->Unit, onClick:()->Unit) {
    Card(
